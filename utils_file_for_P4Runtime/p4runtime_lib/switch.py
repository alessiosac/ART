...
import sys
sys.path.append("/home/p4/Desktop/ROAR22/utils_file_for_P4Runtime/")
...
from v1 import p4runtime_pb2, p4runtime_pb2_grpc
...
class SwitchConnection(object):
    ...
    def UpdateTableEntry(self, table_entry, dry_run=False):
        '''
        print "UpdateTableEntry() is called, device_id=", self.device_id
    	print "UpdateTableEntry() is called, table id=", table_entry.table_id
    	print "UpdateTableEntry() is called, old_table id=", old_table_entry.table_id
            updates = []
            for req in reversed(self.requests):
              for update in reversed(req.updates):
                if update.type == p4runtime_pb2.Update.INSERT:
    	...
    	new_req.election_id.low = 1
            for update in updates:
    	  if update.entity.table_entry == old_table_entry: ##might have problem
                update.type = p4runtime_pb2.Update.MODIFY
    	    update.entity.table_entry.CopyFrom(table_entry)
                new_req.updates.add().CopyFrom(update)
    ...
    def StreamDigestMessages(self, digest_id, dry_run=False):
        ...
        if dry_run:
            print ("P4Runtime Read stream digest message: ")
